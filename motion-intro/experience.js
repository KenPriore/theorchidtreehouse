...
$('#replay').onclick=()=>{rush=false;time=reduce?6.5:0;entry=-1;entryClock=0;navigated=false;document.body.classList.remove('entering');playing=!reduce;sync()};
const full=$('#full');
...
let rush=false;$('#door').onclick=()=>{if(reduce||!ready){location.assign('https://theorchidtreehouse.com/?enter=1');return}document.body.classList.add('entering');if(time<6.5){rush=true;playing=true;motionEnabled=true;sync();return}entryClock=0;entry=0};
...
function draw(now){const dt=last?Math.min((now-last)/1000,.1):0;last=now;if(!document.hidden){if(playing)time+=dt*(rush?(time<3.5?9:2.4):1);if(rush&&time>=6.5&&entry<0){rush=false;entryClock=0;entry=0}if(entry>=0)entry=entryClock+=dt;}const d=Math.min(devicePixelRatio,1.75),w=Math.round(innerWidth*d),h=Math.round(innerHeight*d);if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h)}gl.uniform2f(ur,w,h);gl.uniform1f(uc,motionEnabled?1:0);gl.uniform1f(ut,time);gl.uniform1f(ue,entry);gl.drawArrays(gl.TRIANGLES,0,6);
const ratio=1672/941,sw=Math.min(innerWidth,innerHeight*ratio),sh=sw/ratio;stage={x:(innerWidth-sw)/2,y:(innerHeight-sh)/2,w:sw,h:sh};const door=$('#door');door.hidden=entry>=0;$('#arrival').style.opacity=entry<0?0:Math.max(0,Math.min(1,(entry-.75)/.6));
if(entry>=1.4&&!navigated){navigated=true;location.assign('https://theorchidtreehouse.com/?enter=1');}requestAnimationFrame(draw)}
